    ...
    formCliente.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (clienteEnEdicionId) {
                ...
                // Si cambias el contrato, actualizamos la cuenta por cobrar (si aún no la han pagado)
                if (datosCliente.valor_total > 0) {
                    await clienteSupabase
                        ...
                        .eq('estado', 'PENDIENTE') // Solo actualizamos lo que está pendiente
                        .eq('tipo', 'INGRESO');
                }
                // ---------------------------------------
                ...
                // AUTOMATIZACIÓN FINANCIERA (Crear deuda nueva)
...
// ==========================================
// 12. MÓDULO FINANCIERO (TIMELINE Y P&L) - ACTUALIZADO
// ==========================================

// Variable temporal para guardar deudas descargadas al abrir el modal
...
function abrirModalTransaccion(tipo) {
    ...
    if(tipo === 'INGRESO') {
        ...
        // Si es GASTO, ocultamos la vinculación de cliente (por ahora)
...
async function cargarDeudasClienteFinanzas() {
    ...
    if(deudasClienteCache.length > 0) {
        ...
        selectDeudas.innerHTML += '<option value="NUEVO">✨ Es un ingreso nuevo (No estaba en el plan)</option>';
...
// CARGAR DASHBOARD FINANCIERO (Lógica de Gráficos)
